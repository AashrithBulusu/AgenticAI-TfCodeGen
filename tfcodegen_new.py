...
# Semantic Kernel 1.x imports
from semantic_kernel import Kernel
from semantic_kernel.connectors.ai.open_ai import AzureChatCompletion
# For kernel_function decorator
...
@kernel_function()
async def generate_variables_tf(module_code: str) -> str:
    return f"""
You are an expert Terraform developer. Given the following Terraform module source code:
---
{module_code}
---
Generate a valid variables.tf file for this module, using correct HCL syntax, types, and default values. Output only the variables.tf file content.
"""

@kernel_function()
async def generate_outputs_tf(module_code: str, resource_name: str) -> str:
    return f"""
You are an expert Terraform developer. Given the following Terraform module source code:
---
{module_code}
---
Extract all output blocks from this module. For each output, rewrite it so that the value references the output from the module block in the root module, assuming the module is named \"{resource_name}\".
"""

@kernel_function()
async def fix_terraform_code(code: str, error_output: str) -> str:
    return f"""
    ...
    async def run(self):
        ...
        for iteration in range(self.max_iterations):
            ...
            for res in resources:
                ...
                try:
                    variables_content = await self.kernel.invoke(
                ...
                try:
                    outputs_content = await self.kernel.invoke(
